...
function findDuplicateCustomer(list, data){
  const name = normTextDup(data?.name);
  const phone = normalizePhone(data?.phone);
  const id = data?.id || null;
  if(!name && !phone) return '';
  const dupName = name ? list.find(x=>x.id!==id && normTextDup(x.name)===name) : null;
  if(dupName) return `Zákazník „${dupName.name}“ už v seznamu existuje.`;
  const dupPhone = phone ? list.find(x=>x.id!==id && normalizePhone(x.contactPhone)===phone) : null;
  if(dupPhone) return `Telefonní číslo už je uložené u zákazníka „${dupPhone.name||'bez názvu'}“.`;
  return '';
}
function findDuplicateHotel(list, data){
  const hotel = normTextDup(data?.hotel);
  const phone = normalizePhone(data?.phone);
  const id = data?.id || null;
  if(!hotel && !phone) return '';
  const dupHotel = hotel ? list.find(x=>x.id!==id && normTextDup(x.hotel)===hotel) : null;
  if(dupHotel) return `Hotel „${dupHotel.hotel}“ už v seznamu existuje.`;
  const dupPhone = phone ? list.find(x=>x.id!==id && normalizePhone(x.phone)===phone) : null;
  if(dupPhone) return `Telefonní číslo už je uložené u hotelu „${dupPhone.hotel||'bez názvu'}“.`;
  return '';
}
function findDuplicateFixed(list, data){
  const code = normKeyDup(data?.code);
  const param = normKeyDup(data?.param);
  const id = data?.id || null;
  if(code){
    const dupCode = list.find(x=>x.id!==id && normKeyDup(x.code)===code);
    if(dupCode) return `Pevný kód „${dupCode.code}“ už v seznamu existuje.`;
  }
  if(param){
    const dupParam = list.find(x=>x.id!==id && normKeyDup(x.param)===param);
    if(dupParam) return `Parametr „${dupParam.param}“ už v seznamu existuje.`;
  }
  return '';
}
...
document.getElementById('c-add').addEventListener('click', ()=>{
  openDialog(t("dlg_add_customer_title"), `<label>${t("label_name")}</label><input id="f-name"><label>${t("label_contact")}</label><input id="f-contact"><label>${t("label_phone")}</label><input id="f-phone" inputmode="tel" autocomplete="tel"><label>${t("label_address")}</label><input id="f-address"><label>${t("label_note")}</label><input id="f-note">`, (close)=>{
    ...
    const dup=findDuplicateCustomer(list,{name, phone});
    if(dup){ toast(dup); return; }
    list.push({id:uuid(), name, contactName:(document.getElementById('f-contact').value||'').trim(), contactPhone:phone, address:(document.getElementById('f-address').value||'').trim(), note:(document.getElementById('f-note').value||'').trim(), machines:[]});
...
document.getElementById('c-import-file').addEventListener('change', (e)=>{
  ...
  if(!file) return;
  const mergeMachines = (cur, inc)=>{
    if(!Array.isArray(cur.machines)) cur.machines = [];
    const mByLabel = new Map(cur.machines.map(m=>[normTextDup(m.label), m]).filter(x=>x[0]));
    inc.machines.forEach(im=>{
      const mk = normTextDup(im.label);
      ...
      if(!mk && im.number!=null && cur.machines.some(em=>em.number===im.number)) return;
      const added = { id: im.id || uuid(), label: im.label, number: (im.number!=null && !Number.isNaN(im.number)) ? im.number : null };
      cur.machines.push(added);
      if(mk) mByLabel.set(mk, added);
    });
  };
  const mergeCustomer = (cur, inc)=>{
  ...
  reader.onload = (ev)=>{
    try{
      ...
      const incoming = incomingRaw.map(c=>({
        ...
        machines: Array.isArray(c.machines) ? c.machines.map(m=>({ id: m.id || uuid(), label: String(m.label||'').trim(), number: m.number ? parseInt(m.number,10) : null })) : []
      })).filter(c=>c.name);
      const cleanIncoming = [];
      ...
      incoming.forEach(inc=>{
        const nameKey = normTextDup(inc.name);
        const phoneKey = normalizePhone(inc.contactPhone);
        const cur = (nameKey && incomingByName.get(nameKey)) || (phoneKey && incomingByPhone.get(phoneKey));
        if(cur){ duplicateCount++; mergeCustomer(cur, inc); return; }
        cleanIncoming.push(inc);
        if(nameKey) incomingByName.set(nameKey, inc);
        if(phoneKey) incomingByPhone.set(phoneKey, inc);
      });
      if(customersImportMode === 'override'){
        if(saveCustomers(cleanIncoming)){
          renderCustomers(document.getElementById('c-search').value);
          toast(`Import hotov (Override)${duplicateCount ? ` – sloučeno ${duplicateCount} duplicit` : ''}`);
        }
      }else{
        ...
        existing.forEach(c=>{
          const nk = normTextDup(c.name);
          const pk = normalizePhone(c.contactPhone);
          if(nk) byName.set(nk, c);
          if(pk) byPhone.set(pk, c);
        });
        let addedCount = 0;
        let mergedCount = duplicateCount;
        cleanIncoming.forEach(inc=>{
          const nameKey = normTextDup(inc.name);
          const phoneKey = normalizePhone(inc.contactPhone);
          const cur = (nameKey && byName.get(nameKey)) || (phoneKey && byPhone.get(phoneKey));
          if(cur){ mergedCount++; mergeCustomer(cur, inc); return; }
          existing.push(inc); addedCount++;
          if(nameKey) byName.set(nameKey, inc);
          if(phoneKey) byPhone.set(phoneKey, inc);
        });
        if(saveCustomers(existing)){
          renderCustomers(document.getElementById('c-search').value);
          toast(`Import hotov (Update) – přidáno ${addedCount}${mergedCount ? `, sloučeno ${mergedCount} duplicit` : ''}`);
        }
      }
...
document.getElementById('c-list').addEventListener('click',(e)=>{
  ...
  if(act==='edit-c-btn'){
    ...
    openDialog(
      ...
      (close)=>{
        const name=document.getElementById('f-name').value.trim();
        const phone=(document.getElementById('f-phone').value||'').trim();
        const dup=findDuplicateCustomer(list,{id:c.id, name, phone});
        if(dup){ toast(dup); return; }
        c.name=name;
  ...
  if(act==='edit-c'){ const id=trg.getAttribute('data-id'); const c=list.find(x=>x.id===id);
    openDialog(t("dlg_edit_customer_title"), `<label>${t("label_name")}</label><input id="f-name" value="${escapeHtml(c.name)}"><label>${t("label_contact")}</label><input id="f-contact" value="${escapeHtml(c.contactName||"")}"><label>${t("label_phone")}</label><input id="f-phone" inputmode="tel" value="${escapeHtml(c.contactPhone||"")}"><label>${t("label_address")}</label><input id="f-address" value="${escapeHtml(c.address||"")}"><label>${t("label_note")}</label><input id="f-note" value="${escapeHtml(c.note||"")}">`, (close)=>{
      const name=document.getElementById('f-name').value.trim(); const phone=(document.getElementById('f-phone').value||'').trim(); const dup=findDuplicateCustomer(list,{id:c.id, name, phone}); if(dup){ toast(dup); return; } c.name=name; c.contactName=(document.getElementById('f-contact').value||'').trim(); c.contactPhone=phone; c.address=(document.getElementById('f-address').value||'').trim(); c.note=document.getElementById('f-note').value.trim(); if(saveCustomers(list)){ renderCustomers(document.getElementById('c-search').value); close(); toast(t("toast_saved")); }
    });
    attachPhoneFormatter(document.getElementById('f-phone'));
    return; }
...
if(hDlgSave){
  hDlgSave.addEventListener('click', (ev)=>{
    ...
    const list = loadHotels();
    const dup = findDuplicateHotel(list, {id:hDlgId, hotel, phone});
    if(dup){ toast(dup); return; }
    if(hDlgId){
...
if(fAddBtn){
  fAddBtn.addEventListener('click', ()=>{
    ...
    saveBtn.onclick = (ev)=>{
      ...
      const list = loadFixed();
      const dup = findDuplicateFixed(list, { code: hasCode ? code : '', param: hasParam ? param : '' });
      if(dup){ toast(dup); return; }
  ...
  fListRoot.addEventListener('click', (e)=>{
    ...
    if(act === 'edit-fixed-btn'){
      ...
      saveBtn.onclick = (ev)=>{
        ...
        if(!desc){ toast('Vyplňte popis'); return; }
        if((hasCode && hasParam) || (!hasCode && !hasParam)) { toast('Vyplňte buď kód nebo parametr'); return; }
        const dup = findDuplicateFixed(cur, { id, code: hasCode ? code : '', param: hasParam ? param : '' });
        if(dup){ toast(dup); return; }
        it.label = '';
    ...
    reader.onload = (ev)=>{
      try{
        ...
        const incoming = raw.map(k=>{
          const code = norm(k.code);
          const param = norm(k.param);
          const desc = norm(k.description||k.desc||k.label||k.name);
          const hasParam = !!param || String(k.type||'').toLowerCase()==='param' || String(k.type||'').toLowerCase()==='parameter';
          return { id: k.id || uuid(), label: '', code: hasParam ? '' : code, param: hasParam ? (param || code) : '', description: desc, type: hasParam ? 'param' : 'fixed' };
        }).filter(k=>(k.code || k.param) && k.description);
        const cleanIncoming = [];
        const byCode = new Map();
        const byParam = new Map();
        let duplicateCount = 0;
        incoming.forEach(inc=>{
          const ck = normKeyDup(inc.code);
          const pk = normKeyDup(inc.param);
          const cur = (ck && byCode.get(ck)) || (pk && byParam.get(pk));
          if(cur){ duplicateCount++; if(!cur.description && inc.description) cur.description = inc.description; return; }
          cleanIncoming.push(inc);
          if(ck) byCode.set(ck, inc);
          if(pk) byParam.set(pk, inc);
        });
        if(fixedImportMode === 'override'){
          if(!confirm('Import přepíše existující pevné kódy. Pokračovat?')) return;
          if(saveFixed(cleanIncoming)){
          ...
          existing.forEach(k=>{
            const ck = normKeyDup(k.code);
            const pk = normKeyDup(k.param);
            if(ck) existingByCode.set(ck, k);
            if(pk) existingByParam.set(pk, k);
          });
          let addedCount = 0;
          let mergedCount = duplicateCount;
          cleanIncoming.forEach(inc=>{
            const ck = normKeyDup(inc.code);
            const pk = normKeyDup(inc.param);
            const cur = (ck && existingByCode.get(ck)) || (pk && existingByParam.get(pk));
            if(cur){ mergedCount++; if(!cur.description && inc.description) cur.description = inc.description; return; }
            existing.push(inc); addedCount++;
            if(ck) existingByCode.set(ck, inc);
            if(pk) existingByParam.set(pk, inc);
          });
      ...
      }catch(err){ alert('Import selhal: ' + (err && err.message ? err.message : err)); }
      finally{ fImportInput.value = ''; }
    };
    reader.readAsText(file);
  });
}
...
document.addEventListener("DOMContentLoaded", ()=>{
    document.getElementById("popupCreateCustomer")?.addEventListener("click", ()=>{
        ...
        let customers=loadCustomersLS();
        const dup=findDuplicateCustomer(customers,{name, phone});
